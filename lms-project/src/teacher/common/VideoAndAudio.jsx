...
import {
  ...
  useTrackWatchTimeMutation, //watchTime
...
const useWatchTimeTracker = (
  ...
  // Save actual watch time to backend ithanu nammal kanunna Actual time backendil send cheyyunnathu
...
const useVideoControls = (
  ...
  //progress mutations start
  const [trackProgress] = useTrackProgressMutation();
  //progress mutations finish
  ...
  const togglePlay = useCallback(() => {
    //👉 Browser immediately: togglePlay() call ചെയ്യും Step 1: function START (always first) const videoElement = videoRef.current;
    //HTML <video> element memory-ൽ നിന്നും എടുക്കുന്നു  React state ഒന്നും ഇതുവരെ മാറിയിട്ടില്ല
    const videoElement = videoRef.current;
    ...
    if (isPlaying) {
      ...
      //video.pause() ennal browser levelil ithu trigger akum handlePause()
    } else {
  ...
  //angane namma mouse down cheythu drag cheythu upcheythappol ivide ethum
  const seekTo = useCallback(
    async (seconds) => {
      ...
      if (user && Math.abs(newTime - oldTime) > 0.5) {
        ...
        // 🔥 Now trackProgress properly 10 second
        await trackProgress({
          type: seekType,
          userId: user._id,
          subjectId: course.subjectId,
...
// ivideyanu yethartha entry point 1
const EnhancedVideoPlayer = ({
  ...
  const toggleFullscreen = () => {
    // Toggle CSS fullscreen, NOT browser fullscreen
    const newFullscreenState = !isFullscreen;
    setIsFullscreen(newFullscreenState);

    // Ensure video plays properly in fullscreen
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case "f":
          e.preventDefault();
          toggleFullscreen(); // This now uses CSS fullscreen
          break;
        case "m":
          e.preventDefault();
          toggleMute();
        ...
        case "escape":
          if (showSettings || showResources) {
            ...
            // Exit CSS fullscreen, NOT browser fullscreen
  ...
  useEffect(() => {
    // Fullscreen-ൽ എപ്പോഴും കഴ്സർ കാണിക്കാൻ
    if (isFullscreen) {
      document.body.style.cursor = "auto";
    } else {
      // Normal mode-ൽ മാത്രം hide ചെയ്യുക
  ...
  // Handle seek ennathu user ivide progress bar mouse down cheythathinu shesham drag cheyyunna bhagam
  //👉 UI update only
  //👉 video still paused
  ...
  //user progressbar eppol mouse Down cheyumo appol ithu call cheyyum
  //👉 user drag ചെയ്യുമ്പോൾ video pause
  // 👉 wrong watch-time avoid ചെയ്യാൻ
  ...
  //User mouse വിട്ടാൽ (MOST IMPORTANT)
  //setSeeking(false);
  //seekTo(newTime);
  //video.play();
  ...
  const resumePositionRatio =
    userVideoState?.lastPlayedTime && duration > 0
      ...
      : 0;
  // bro here statrt main entry point
  return (
    <div
    ...
    ${isFullscreen ? "fixed inset-0 z-[100]" : "max-w-6xl mx-auto"}`}
    ...
    >
      ...
      {/* Video Container */}
      <div
        className={`relative w-full ${isFullscreen ? "h-screen" : "h-auto"} flex items-center justify-center bg-black`}
      >
        <video
          ref={videoRef}
        ...
        {/* Center Play Button ithanu centeril play cheyyan vendi nikkana play before */}
        {!isPlaying &&
          !isPreviewLimitReached &&
          !userVideoState?.lastPlayedTime && (
            <button
        ...
        {/* Quick Actions Bar ithu video playerinte akathulla sidebar ayi thonnunna buttons */}
        <AnimatePresence>
          {showControls && !isPreviewLimitReached && (
            <motion.div
              ...
              className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-20 bg-red-500 "
            >
              {[
          ...
          >
            ...
            <div className="relative mb-6 group">
              ...
              <input
                ...
                onPointerDown={handleSeekStart} // 👈 FIX
                onPointerUp={handleSeekEnd} // 👈 FIX
