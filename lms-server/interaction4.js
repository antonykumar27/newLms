// controllers/completeInteractionController.js
...
const ACTIVITY_SCORING = { PAGE_VISIT: 5, QUIZ_ATTEMPT: 10, WATCH_MINUTE: 2 };
...
const calculateIntensity = (type, currentTime, duration) => {
...
const isValidForStreak = (type, currentTime, duration) => {
  if (
...
const updateDailyActivity = async (
  ...
  try {
    ...
    dailyActivity.activities.push({
      ...
      chapterId,
      pageId,
      metadata: { currentTime: data?.currentTime, duration: data?.duration },
    });
    ...
    if (isValidForStreak(type, data?.currentTime, data?.duration))
...
const updateHeatmap = async (
  ...
  try {
    ...
    const intensity = calculateIntensity(type, currentTime, duration);
    ...
    if (dayData) {
      ...
      dayData.color = HEATMAP_COLORS[dayData.intensity] || HEATMAP_COLORS[0];
    }
...
const updateStreak = async (userId, type, currentTime, duration, today) => {
  try {
    if (!isValidForStreak(type, currentTime, duration)) return null;
    ...
    if (!streak) {
      streak = new StudentStreak({
        ...
        streakFreeze: { isActive: false, freezeCount: 3 },
    ...
    const lastActive = streak.lastActiveDate
      ...
      : null;
    if (lastActive === currentDate.getTime()) return streak;
    ...
    if (lastActive === yesterday.getTime()) {
      ...
      const daysMissed =
        Math.floor(
          (currentDate.getTime() - lastActive) / (1000 * 60 * 60 * 24),
        ) - 1;
...
const updateInsights = async (
  ...
  try {
    ...
    const activityHour = new Date(today).getHours();
    if (activityHour < 12)
...
const updateWatchTimeProgress = async (
  ...
  try {
    ...
    if (isFirstTime) updateData.$inc.uniqueWatchedSeconds = watchedSeconds;
    else if (currentTime > (watchTime?.lastPosition || 0)) {
      const uniqueSeconds = Math.min(
        watchedSeconds,
        currentTime - (watchTime?.lastPosition || 0),
      );
    ...
    if (watchTime) {
      const finalSessionId = sessionId || generateSessionId(userId, videoId);
      const sessionStartTime = Math.max(0, currentTime - watchedSeconds);
...
// ==============================================
// MAIN EXPORTED FUNCTION - SINGLE POST ENDPOINT
// ==============================================

exports.trackCompleteInteraction = async (req, res) => {
  ...
  try {
    ...
    console.log(`📝 Processing interaction: ${type} for user: ${userId}`);
    ...
    // ✅ STEP 2: GET STANDARD ID
    const subjectDoc = await StandardSubject.findById(subjectId)
      .select("standardId")
      .session(session);
    if (!subjectDoc) {
      return res
    ...
    if (existingInteraction) {
      ...
      sessionId = generateSessionId(userId, videoId);
    }

    // ✅ STEP 7: CREATE AND SAVE INTERACTION
    const interactionData = {
      ...
      data: {
        ...
        ...(type === "video_seek" && {
          ...
          seekAmount:
            data.seekAmount ||
            (data.toTime && data.fromTime ? data.toTime - data.fromTime : null),
          seekDirection:
            data.seekDirection ||
    ...
    // ✅ STEP 8: UPDATE USER LAST ACTIVITY
    await User.findByIdAndUpdate(userId, {
      lastActive: now,
      $inc: { "analytics.totalInteractions": 1 },
    }).session(session);

    // ✅ STEP 9: HANDLE QUIZ COMPLETION IF APPLICABLE
    let quizAttempt = null;
    if (
      (type === "quiz_complete" || isQuizComplete) &&
      quizId &&
      quizScore !== undefined
    ) {
      quizAttempt = await handleQuizCompletion(
        userId,
        quizId,
        quizScore,
        quizAnswers,
    ...
    // ✅ STEP 10: RUN ALL BACKGROUND TASKS IN PARALLEL
    const backgroundTasks = [
      ...
      updateDailyActivity(
        ...
        chapterId,
        pageId,
        { currentTime, duration: totalDuration, watchTimeMs },
        today,
      ),

      // Heatmap
      updateHeatmap(userId, type, currentTime, totalDuration, subjectId, today),

      // Streak
      updateStreak(userId, type, currentTime, totalDuration, today),
    ...
    // ✅ STEP 11: GET STREAK DATA FOR BADGE CHECK
    const streak = await StudentStreak.findOne({ studentId: userId });

    // ✅ STEP 12: CHECK BADGES
    ...
    // ✅ STEP 14: UPDATE LEADERBOARD (fire and forget)
    updateLeaderboardData(standardId).catch((err) =>
      console.error("Leaderboard update failed:", err),
    );

    // ✅ STEP 15: CALCULATE PAGE PROGRESS
    let pageProgress = null;
    if (pageId) {
      pageProgress = await calculatePageProgress(userId, pageId);
    }

    // ✅ STEP 16: CALCULATE CHAPTER PROGRESS
    let chapterProgress = null;
    if (chapterId) {
      chapterProgress = await calculateChapterProgress(userId, chapterId);
    }

    // ✅ STEP 17: CALCULATE SUBJECT PROGRESS
    ...
    // ✅ STEP 18: FETCH UPDATED DATA FOR RESPONSE
    const [watchTimeData, recentBadges] = await Promise.all([
      pageId && videoId
        ? WatchTime.findOne(
            { userId, videoId, page: pageId },
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        progress: {
          page: pageProgress
            ? {
                isComplete: pageProgress.isComplete,
                percentage: pageProgress.progressPercentage,
                videoCompleted: pageProgress.videoProgress?.isCompleted,
                quizCompleted: pageProgress.quizProgress?.isCompleted,
                quizScore: pageProgress.quizProgress?.bestScore,
              }
            : null,

          chapter: chapterProgress
            ? {
                isComplete: chapterProgress.isComplete,
                pagesCompleted: chapterProgress.pageStats?.completed,
                totalPages: chapterProgress.pageStats?.total,
                quizPassed: chapterProgress.chapterQuiz?.isPassed,
                quizScore: chapterProgress.chapterQuiz?.bestScore,
                conceptScore: chapterProgress.conceptUnderstandingScore,
              }
            : null,

          subject: subjectProgress
            ? {
                isComplete: subjectProgress.isComplete,
                chaptersCompleted: subjectProgress.chapterStats?.completed,
                totalChapters: subjectProgress.chapterStats?.total,
                quizPassed: subjectProgress.subjectQuiz?.isPassed,
                quizScore: subjectProgress.subjectQuiz?.bestScore,
              }
            : null,
        },
        ...
        watchTime: watchTimeData
          ? {
              totalSeconds: watchTimeData.totalWatchedSeconds || 0,
              uniqueSeconds: watchTimeData.uniqueWatchedSeconds || 0,
        ...
        milestone: streak?.milestones?.find(
          (m) =>
            m.days === streak.currentStreak &&
            new Date(m.achievedAt).toDateString() === new Date().toDateString(),
        )
          ? {
              message: `🎉 Congratulations! You've reached a ${streak.currentStreak}-day streak!`,
              days: streak.currentStreak,
            }
          : null,
    ...
    console.error("❌ Complete interaction error:", error);
